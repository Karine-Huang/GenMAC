...
Condition_gen = namedtuple(
    "Condition", ["prompt", "boxes", "phrases", "object_positions", "token_map", "generation_param_dic"]
)

Condition_gen_prompt = namedtuple(
    "Condition", ["prompt", "boxes", "phrases", "object_positions", "token_map", "generation_param_dic", "new_prompt"]
)
...
def parsed_layout_to_condition(
    ...
    if use_generation:
        generation_param_dic = {"increase":[], "decrease":[]}
        generation_suggestion = parsed_layout["Generation suggestion"]

        emphasize_pattern = r'emphasize id (\d+(?:, id \d+)*)'
        emphasize_matches = re.findall(emphasize_pattern, generation_suggestion)
        emphasize_ids = []
        for match in emphasize_matches:
            emphasize_ids.extend(re.findall(r'\d+', match))

        deemphasize_pattern = r'deemphasize id (\d+(?:, id \d+)*)'
        deemphasize_matches = re.findall(deemphasize_pattern, generation_suggestion)

        deemphasize_ids = []
        for match in deemphasize_matches:
            deemphasize_ids.extend(re.findall(r'\d+', match))

        emphasize_ids = [int(id) for id in emphasize_ids]
        deemphasize_ids = [int(id) for id in deemphasize_ids]


        for id in emphasize_ids:
            generation_param_dic["increase"].append(int(id))
        for id in deemphasize_ids:
            generation_param_dic["decrease"].append(int(id))
    if use_new_prompt:
        new_prompt = parsed_layout["New prompt"]
        prompt = new_prompt
    ...
    if use_generation and use_new_prompt:
        return Condition_gen_prompt(prompt, boxes, phrases, object_positions, token_map, generation_param_dic, new_prompt)
            
    elif use_generation:
        return Condition_gen(prompt, boxes, phrases, object_positions, token_map, generation_param_dic)
