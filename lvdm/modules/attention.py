...
class CrossAttention(nn.Module):
    ...
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0., 
        ...
        self.cross_attention_mode = context_dim is not None
        ...
        if self.relative_position:
            ...
            self.forward = self.efficient_comp_forward #TODO
    ...
    # def efficient_comp_forward(self, x, context=None, mask=None):

    def efficient_comp_forward(
        self,
        x,
        context=None,
        mask=None,
       ...
       # ------------------------------
       # judge to fast compute or not
        cross_attn = context is not None

        if (not cross_attn) or (
            not (
                save_attn_to_dict is not None
                # and (save_keys is None or (tuple(attn_key) in save_keys))
                and (save_keys is None or (attn_key in save_keys))
            )
            and not return_attntion_probs
        ):  
                
            return self.efficient_forward(x, context=context)
        # ------------------------------
        ...
        ## Videocrafter2 adopts a different way from diffusers to compute negative/unconditional prompts. It computes conditioned/unconditional cross attention separately. So a filter here is needed as this function will be passed to unconditional prompts.
        if not torch.is_tensor(context):  
            region_diffusion = True
            for m, c in context[0]:
                layout_masks.append(m)
                conditions.append(c)
            context = torch.stack(conditions, dim=0)
 
            context = context.repeat_interleave(q.shape[0], dim=0)
            q = q.repeat(len(layout_masks), 1, 1)
        ...
        # k = k.transpose(1, 2)
        # v = v.transpose(1, 2)
        attention_map = q @ k.transpose(-2, -1)
        attention_map = attention_map.softmax(-1) # get attention map ()
        # attention_map = F.dropout(attention_map, p)
        # attention_map = attention_map @ v  # same as out
        attention_probs_before_process = attention_map
        ...
        if return_attntion_probs or save_attn_to_dict is not None:
            ...
            attention_probs_unflattened = attention_probs_before_process.unflatten(
                dim=0, sizes=(batch_size, self.heads)
            )
            if return_token_ca_only is not None:
                ...
                if isinstance(return_token_ca_only, int):
                    ...
                    attention_probs_unflattened = attention_probs_unflattened[
                        :, :, :, return_token_ca_only : return_token_ca_only + 1
                    ]
                else:
            ...
            if return_cond_ca_only:
                assert (
                    batch_size % 2 == 0
                ), f"Samples are not in pairs: {batch_size} samples"
                attention_probs_unflattened = attention_probs_unflattened[
                    batch_size // 2 :
                ]
            ...
            # save_attn_to_dict or save_keys set to None means we do not save cross-attention maps
            if (
                save_attn_to_dict is not None
                and save_keys is not None
                # and (tuple(attn_key) in save_keys)
                and (attn_key in save_keys)
            ):
                save_attn_to_dict[tuple(attn_key)] = attention_probs_unflattened
            if return_attntion_probs:
                return self.to_out(out), attention_probs_unflattened
        # ------------------------------
        return self.to_out(out)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None, mask=None,
        ...
        input_tuple = (x, context, mask, cross_attention_kwargs)
        return checkpoint(self._forward, input_tuple, self.parameters(), self.checkpoint)

    # def _forward(self, x, context=None, mask=None):
    def _forward(self, x, context=None, mask=None, cross_attention_kwargs: Dict[str, Any] = None):

        x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else None, mask=mask, **cross_attention_kwargs) + x

        x = self.attn2(self.norm2(x), context=context, mask=mask, **cross_attention_kwargs,) + x
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None,
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            cross_attention_kwargs["attn_key"] = base_attn_key + [i]
