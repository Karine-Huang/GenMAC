...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, batch_size=None, cross_attention_kwargs=None):
        if cross_attention_kwargs is not None:
            base_attn_key = cross_attention_kwargs["attn_key"]

        for block_ind, layer in enumerate(self):
            if cross_attention_kwargs is not None:
                cross_attention_kwargs["attn_key"] = base_attn_key + [block_ind]
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb, batch_size)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, cross_attention_kwargs=cross_attention_kwargs)
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fps=16, 
        ...
        for id, module in enumerate(self.input_blocks):
            cross_attention_kwargs["attn_key"] = ["down", id]
            if (
                id in (1,2,4,5,7,8) 
            ):
                h = module(
                    h, emb, context=context, batch_size=b,
            ...
            if id ==0 and self.addition_attention:
                # # TODO add
                cross_attention_kwargs["attn_key"] = ["in", 0]
