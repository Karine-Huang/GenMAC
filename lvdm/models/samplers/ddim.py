...
import json
from lvdm.models.pipelines import latent_backward_guidance
...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            img, pred_x0, loss_attn = self.p_sample_ddim(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if x.dim() == 5:
            ...
            is_video = False
        # ---------------------------------------
        # add latent control in this part
        save_keys = []
        ...
        backward_guidance_kwargs = kwargs.get("backward_guidance_kwargs", {})
        ret = latent_backward_guidance(
            self.model,
            c,
            latents=x,
            index=len(self.ddim_timesteps) - 1 - index,
            t=t,
            loss=loss_attn,
            return_saved_attn=False,
            **backward_guidance_kwargs,
        )
        x, loss_attn = ret[0], ret[1]
        
        # ---------------------------------------
        
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            e_t = self.model.apply_model(x, t, c, cross_attention_kwargs, **kwargs) # unet denoiser
        else:
            # with unconditional condition
            if isinstance(c, torch.Tensor):
                e_t = self.model.apply_model(x, t, c, cross_attention_kwargs, **kwargs)
                e_t_uncond = self.model.apply_model(x, t, unconditional_conditioning, cross_attention_kwargs, **kwargs)
        ...
        return x_prev, pred_x0, loss_attn
