...
def load_prompts(prompt_file, prompt_type=None, subset_ratio=0.1):
    if prompt_type == 'subset':
        f = open(prompt_file, 'r')
        prompt_list = []
        for idx, line in enumerate(f.readlines()):
            if idx < 100 * subset_ratio:
                l = line.strip()
                if len(l) != 0:
...
def load_prompts_from_json(prompt_file):
    import json
...
def save_videos(batch_tensors, savedir, filenames, fps=10):
    ...
    for idx, vid_tensor in enumerate(batch_tensors):
        ...
        grid_np = grid.cpu().numpy() if grid.is_cuda else grid.numpy()
        imageio.mimwrite(savepath, grid_np, fps=fps, quality=9)
