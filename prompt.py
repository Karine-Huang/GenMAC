...
templatev_redesign = [ # read from txt.
    "assets/agents_prompt/v1/overall_requirements.txt",
    "assets/agents_prompt/v1/verification.txt",
    "assets/agents_prompt/v1/suggestion.txt",
    "assets/agents_prompt/v1/correction_a1.txt",
    "assets/agents_prompt/v1/correction_b1.txt",
    "assets/agents_prompt/v1/correction_b2.txt",
    "assets/agents_prompt/v1/structured_output.txt",
]


templates = {
    "v0.1": templatev0_1_chat, 
    "v_redesign": templatev_redesign, 
...
required_lines_gen = [f"Frame {i+1}:" for i in range(6)] + ["Background keyword:"] + ["Generation suggestion:"]
required_lines_gen_prompt = [f"Frame {i+1}:" for i in range(6)] + ["Background keyword:"] + ["Generation suggestion:"] + ["New prompt:"]
required_lines_ast = [True] * 6 + [False]
required_lines_ast_gen = [True] * 6 + [False] * 2
required_lines_ast_gen_prompt = [True] * 6 + [False] * 3
...
def get_prompts(prompt_type, return_predicates=False, subset_ratio=0.1):
...
def get_video_path(video_paths, prompt_type="demo"):

    if prompt_type == "demo":
        
        t2v_comp_video_path = [
            os.path.join(video_paths, "results_t2v_baseline_0", "base_512_v2" ),  #TODO
        ]
        total_video_path = {}
        for path in t2v_comp_video_path:
            num = path.split("/")[-2].split("_")[-1]
            try:
                video_path_tmp = os.listdir(path)
                # sort
                video_path_tmp = sorted(video_path_tmp)
                video_path_tmp = [os.path.join(path, video) for video in video_path_tmp]
            except:
                video_path_tmp = []
            total_video_path[num] = video_path_tmp
